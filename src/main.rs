    ...
    impl Solver {
        pub fn random_search() {
                ...
                *to_cnt += 1;
                ...
                for ls in ls {
                    *to1_cnt += 1;
                    ...
                    eval0.push(1.0 - power(q, pw));
                }
                let eval0 = eval0.iter().sum::<f64>()
                    - (eval0.iter().map(|&x| x * x).sum::<f64>() / eval0.len() as f64
                        - (eval0.iter().sum::<f64>() / eval0.len() as f64).powi(2))
                    .sqrt()
                        * (eval0.len() as f64).sqrt();
                let mut eval2 = None;
            ...
            for li in 1.. {
                ...
                if li % 10000 == 0 {
                    let path = std::path::PathBuf::from(format!(r"tbl3\tbl{}.txt", li));
